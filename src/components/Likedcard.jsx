...
export default function Likedcard() {
  const { likedProducts, setLikedProducts } = useContext(Contexts);
  const navigate = useNavigate();

  const likedItems = Object.keys(likedProducts).filter(
    (key) => likedProducts[key]
  );
  ...
  return (
    <div className="bg-white mt-[30px]">
      <div className="w-[90%] m-auto gap-5">
        {likedItems.length > 0 ? (
          ...
          <div className="w-full py-[50px]">
            <p className="text-2xl font-bold text-black w-full text-center">Sizga yoqqani mavjud emas</p>
          </div>
        )}
      </div>
    </div>
